...
setwd("F:/Hunor/phd project/RStudio works/r_data_analysis-22_23_fall/data")

df_main  <- read_excel("F:/Hunor/phd project/RStudio works/r_data_analysis-22_23_fall/data/assignment_3_dataset.xlsx")
...
#Investigating the dataset for possible errors and outliers
```{r}
glimpse(df_main)
...
#Pain 50 should be removed from the original dataset

df_main %>%  
  count(mindfulness) %>% 
  filter(mindfulness > 6)

#These values should be removed from the original dataset

df_main %>% 
  count(sex) 

#the woman sex attribute should be changed
...
# under 40 kg weight seems like something problematic, I would exclude from analysis
```

#Basic plot to check the sex and age data on dataset
```{r}
df_main %>% 
  filter(pain < 10) %>% 
  ggplot +
...
df_cleaned <- df_main %>% 
  ...
  filter(pain < 10, mindfulness < 6, weight > 36)
...
```{r}
model_complex <- lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum + cortisol_saliva, data = df_cleaned)
...
#Creates two plots to investigate the potential outlier values: Residual vs Leverage and Cook's distance plots
```{r}
plot(model_complex,4)
...
```{r}
residuals <- residuals(model_complex) 
...
```{r}
model_complex_nosaliva <- lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum , data = df_cleaned)

summary(model_complex_nosaliva)

vifs2 <- vif(model_complex_nosaliva)
...
```{r}
plot(model_complex_nosaliva,4)

plot(model_complex_nosaliva, which = 5)
```
...
```{r}
residuals2 <- residuals(model_complex_nosaliva) 
...
```{r}
fitted_values2 <- fitted(model_complex_nosaliva)
...
```{r}
model_simple <- lm(pain ~ age + sex, data = df_cleaned)
...
```{r}
model_complex_final <- lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum , data = df_cleaned)

aic_complex <- AIC(model_complex_final)
...
```{r}
lm_test <- anova(model_simple, model_complex_final)
...
#According to the more complex model where we included predictor factors can more complexly describe the relationship between pain and predictors. Since the aic_simple > aic_complex, and the complex model fits significantly better, we should consider including the measurement of psychological and phisiological factors, especially cortisol and pain catastrophising.
