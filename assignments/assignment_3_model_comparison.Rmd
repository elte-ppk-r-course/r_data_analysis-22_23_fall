...
```{r}
pain_experienced <- read_xlsx("../data/assignment_3_dataset.xlsx")
```
...
##### check out the data
```{r}
print(pain_experienced)
```

##### max pain 50, max mindfulness 6.22 => outliers
```{r}
pain_experienced %>%
  summary()
```

##### sex distribution (1 woman => clean data)
```{r}
pain_experienced %>%
  count(sex)
```
...
```{r}
ggplot(pain_experienced, aes(age, pain, color = sex)) +
  geom_point()
```

##### iq distribution
```{r}
ggdensity(pain_experienced$IQ, xlab = "Distributation of IQ")
```

##### weight distribution
```{r}
ggdensity(pain_experienced$weight, xlab = "Distributation of weight")
```
...
```{r}
pain_experienced_clean <- pain_experienced %>%
  mutate(
...
model <- lm(pain ~ sex + age, data = pain_experienced_clean)
print(model)
...
```{r}
complex_model <- lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum + cortisol_saliva, data = pain_experienced_clean)
...
##### there are no too influential outliers
```{r}
plot(complex_model, 5)
```
...
##### cortisol variables are correlated, cortisol_saliva removed from complex_model2
```{r}
VIF(complex_model)
```
...
```{r}
complex_model2 <- lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum, data = pain_experienced_clean)
```

#### Checking outliers of the updated model

```{r}
plot(complex_model2, 5)
```

#### Checking assumptions of the updated model

Normality assumption, Linearity assumption, Homoscedasticty assumption (homogeneity of variance), Multicollinearity assumption

##### found Normality assumption, Linearity assumption, Homoscedasticty assumption (homogeneity of variance), Multicollinearity assumption acceptable
```{r}
...
##### using first model
##### The regression equation: Y = 8.49 + -0.0891 * age + 0.103 * sexmale
```{r}
glance(model)
...
##### using complex_model2
##### The regression equation: Y = -0.97 + 0.20*X1 - 0.12*X2 - 0.04*X3 + 0.27*X4 + 0.35*X5 - 0.09*X6
```{r}
glance(complex_model2)

linear_model_final <-
  tidy(complex_model2, conf.int = TRUE, conf.level = 0.95)
...
```{r}
compare <- anova(model, complex_model2)
compare
```
