...
range(assignment_3_dataset$pain) # Should be between 1-10, the output indicates there is a coding error
...
assignment_3_dataset <- subset(assignment_3_dataset, pain >= 1 & pain <= 10 & mindfulness >= 1 & mindfulness <= 6)
...
```{r}
# Extract predictor variable names from the model formula
predictor_names <- attr(terms(complex_model), "term.labels")

# Calculate VIF
vif_values <- vif(complex_model)

# Create data frame with predictor variable names and VIF values
vif_results <- data.frame(variable = predictor_names, vif = vif_values)

# Display the VIF results
print(vif_results) # cortisol_serum and cortisol_saliva are both over 5
```
...
```{r}
# Extract predictor variable names from the model formula
updated_predictor_names <- attr(terms(updated_complex_model), "term.labels")

# Calculate VIF
updated_vif_values <- vif(updated_complex_model)

# Create data frame with predictor variable names and VIF values
updated_vif_results <- data.frame(updated_variable = updated_predictor_names, updated_vif = updated_vif_values)

# Display the VIF results
print(updated_vif_results) # Now cortisol_serum is reduced to 1.533348 
```
