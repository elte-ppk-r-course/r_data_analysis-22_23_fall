...
pain_experience <- read_excel(file.choose())
```
...
#age is a significant main effect (p0.001) but not gender in our model, which is significant (F(2,156) = 7.511, p 0.001, AIC = 574.196).

#the dependent variable's explained variance is **adj R2 = 0.07614**, or 7,6%, which is lower than we would like it to be in the more sophisticated model.


#age's confidence interval is [-0.13, -0.04], which excludes zero, but sex's confidence interval is [-0.35, 0.56], which includes zero.

#Y = 8.49145 + -0.08906 * x1 + 0.10287 * x2 is the regression equation.
```
