...
ggplot(assignment_3_dataset, aes(x = age, y = pain, color = sex)) + # Creating a scatter plot
  geom_point() +
  labs(x = "Age", y = "Pain", color = "Sex") +
  ggtitle("Scatter Plot: Age vs. Pain with Sex") # As we have found earlier, we have some problems with the dataset
...
ggplot(assignment_3_dataset, aes(x = age, y = pain, color = sex)) +
  geom_count(alpha=0.5) + # Adjust transparency, to see overlapping points
  labs(x = "Age", y = "Pain", color = "Sex") +
  ggtitle("Data cleaned Scatter Plot: Age vs. Pain with Sex") +
  scale_y_continuous(breaks = seq(0, 10), limits = c(0, 10)) + # Adjust y scale to see our full pain measurement scale
  xlim(0, 100) # Adjust x scale to better understand the age of our control group
...
df_model <- model_summary$fstatistic[2]
df_residual <- model_summary$fstatistic[3]
p_value <- pf(f_value, df_model, df_residual, lower.tail = FALSE)

# Create a table of coefficients
coefficients_table <- data.frame(
  Coefficient = coef(simple_model),
  `95% CI` = confint(simple_model),
  `Std. Coefficient (Beta)` = coef(simple_model) / sd(assignment_3_dataset$pain),
  p_value = coef(summary(simple_model))[, "Pr(>|t|)"]
)

# Print model test statistics
...
# Extract model summary
model_summary2 <- summary(updated_complex_model)

# Extract adjusted R-squared
adj_r_squared2 <- model_summary2$adj.r.squared

# Extract F-statistic, degrees of freedom, and p-value
f_value2 <- model_summary2$fstatistic[1]
df_model2 <- model_summary2$fstatistic[2]
df_residual2 <- model_summary2$fstatistic[3]
p_value2 <- pf(f_value2, df_model2, df_residual2, lower.tail = FALSE)

# Create a table of coefficients
coefficients_table2 <- data.frame(
  Coefficient = coef(updated_complex_model),
  `95% CI` = confint(updated_complex_model),
  `Std. Coefficient (Beta)` = coef(updated_complex_model) / sd(assignment_3_dataset$pain),
  p_value = coef(summary(updated_complex_model))[, "Pr(>|t|)"]
)

# Print model test statistics
cat("Adjusted R-squared:", adj_r_squared2, "\n")
cat("F-statistic:", f_value2, "\n")
cat("Model degrees of freedom:", df_model2, "\n")
cat("Residual degrees of freedom:", df_residual2, "\n")
