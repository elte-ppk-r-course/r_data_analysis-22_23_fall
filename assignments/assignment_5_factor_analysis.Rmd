...
__ar1-ar28__ contain the data from the 28 items of the ARS. Participants had to rate their agreement with each statement separately on a 1-5 Likert scale with the following anchors: 1 - strongly disagree, 2 â€“ disagree, 3 - no opinion, 4 â€“ agree, 5 - strongly agree.
...
Wuensch, K. L., Jenkins, K. W., & Poteat, G. M. (2002). Misanthropy, idealism, and attitudes towards animals. _AnthrozoÃ¶s, 15_, 139-149
...
__sex:__ The self reported sex of the participant. This is a categorical variable coded as 1 â€“ female, 2 â€“ male.

__party:__ Self reported party affiliation of the person (in the USA). This is a categorical variable coded as 1 - democrat, 2 - republican, 3 - other, 4 â€“ none.
...
When you have arrived at the factor structure you consider final, give names to the factors you derived from the data. Save the factor scores and build a linear regression model to predict how conservative or liberal participants are (using the â€œliberalâ€ variable as a dependent variable) with the factors you identified as the predictors.
...
```{r}
animalRightsScalePath <- if (file.exists("./data/assignment_5_dataset.csv")) { "./data/assignment_5_dataset.csv" } else { "https://raw.githubusercontent.com/elte-ppk-r-course/r_data_analysis-22_23_fall/main/data/../data/assignment_5_dataset.csv" }
animalRightsScale <- read_csv(animalRightsScalePath)
```
...
dim(animalRightsScale)
stat.desc(animalRightsScale)
describe(animalRightsScale)
```
...
```{r}
animalRightsScale <- animalRightsScale %>%
...
```{r}
animalRightsScaleItems <- animalRightsScale %>% select(matches("ar[0-9]+"))
```
...
correlationMatrix <- correlate(animalRightsScaleItems)
```
...
```{r}
rplot(correlationMatrix, print_cor = TRUE)
```
...
library(mvnTest)
HZ.test(na.omit(animalRightsScaleItems))
```
...
skewness(animalRightsScaleItems)
kurtosis(animalRightsScaleItems)
```
...
```{r}
prcomp(drop_na(animalRightsScaleItems), scale. = FALSE)
abline(h=1)
# Suggests 7 factors

fa.parallel(animalRightsScaleItems)
# Suggests 4 factors
...
```{r}
animalRights7Factors <- fa(animalRightsScaleItems, nfactors = 7, rotate = "varimax")
animalRights4Factors <- fa(animalRightsScaleItems, nfactors = 4, rotate = "varimax")
```

Sort the communality scores in decreasing order.

```{r}
sort(animalRights7Factors$communality, decreasing = TRUE)
sort(animalRights4Factors$communality, decreasing = TRUE)
```

Calculate the mean communality scores.

```{r}
mean(animalRights7Factors$communality)
mean(animalRights4Factors$communality)
```

Show the factor loadings for the chosen factor structure.

```{r}
animalRights7Factors$loadings
animalRights4Factors$loadings
```

Visualize the factor structure.

```{r}
fa.diagram(animalRights7Factors)
fa.diagram(animalRights4Factors)

  # Rational-scientific control
  # * __ar 17.__ There are plenty of viable alternatives to the use of animals in biomedical and behavioral research.
  # * __ar 6.__ Most medical research done on animals is unnecessary and invalid.
  # * __ar 2.__ Animal research cannot be justified and should be stopped.
  # * __ar 27.__ Most psychological research done on animals is unnecessary and invalid.
...
```{r}
animalRights7FactorsScores <- animalRights7Factors$scores
animalRights4FactorsScores <- animalRights4Factors$scores
```

Bind factor scores to the original dataset.

```{r}
animalRightsScaleWith4Factors <- animalRightsScale %>%
  cbind(animalRights4FactorsScores) %>%
  rename(moralSocial = MR1, rationalScientific = MR2, philosophical = MR3, directConsumptive = MR4)

animalRightsScaleWith7Factors <- animalRightsScale %>%
  cbind(animalRights7FactorsScores)
```

Run the logistic regression.

```{r}
animalRights4FactorModel <- lm(liberal ~ rationalScientific + moralSocial + philosophical + directConsumptive, data = animalRightsScaleWith4Factors)
summary(animalRights4FactorModel)
# Items regarding direct consumption of animal products are the most influential in predicting liberal scores in the 4-factor model.

animalRights7FactorModel <- lm(liberal ~ MR1 + MR5 + MR7 + MR3 + MR4 + MR6 + MR2, data = animalRightsScaleWith7Factors)
summary(animalRights7FactorModel)
# Items 28, 10, 24, 7 (subset of 4-factor model's Direct-Consumptive factor, all of the items from the factor that involves killing animals) remained the most influential.
```
